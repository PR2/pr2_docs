...
PR2/basestation installers previous to version 0.5 used Ubuntu 9.04
(Jaunty Jackalope). 
...
\subsubsection{Initialize the Basestation VPN Server}

Initializing the basestation VPN server is redundant, since the ``robot-brand''
step will do this for you. If you want to generate the key manually, follow the steps below.

The server needs an appropriate key.  To generate the key for the VPN server run:
\begin{verbatim}
pr2admin@basestation$ sudo /etc/wg-openvpn/gen_server_key
\end{verbatim}
...
Note: For PR2's with Ubuntu Jaunty, the script is in \texttt{/etc/openvpn}. 
