...
\subsection{\href{http://www.ros.org/wiki/rosrecord}{rosrecord}}
Records currently published topics and generates a ``.bag'' file (so named for historical reasons).
\begin{tabbing}
E\=x\=amples:\\
\> Record all topics:\\
\> \>\texttt{\$ rosrecord -a}\\
\> Record select topics:\\
\> \>\texttt{\$ rosrecord topic1 topic2}\\
\end{tabbing}

\vspace{-1.5 mm}
\subsection{\href{http://www.ros.org/wiki/rosplay}{rosplay}}
Replays the contents of one or more ``.bag'' files in a time-synchronized fashion.
\begin{tabbing}
E\=x\=amples:\\
\> Replay all messages without waiting:\\
\> \>\texttt{\$ rosplay -a demo\_log.bag}\\
\> Replay several bag files at once:\\
\> \>\texttt{\$ rosplay demo1.bag demo2.bag}\\
\> Check contents of the bag without playing back:\\
\> \>\texttt{\$ rosplay -c demo1.bag}\\
\end{tabbing}

\vspace{-2.5 mm}
\section{Graphical Command-line Tools}
...
\vspace{2.5 mm}
\includegraphics[width=\columnwidth]{rxplot.png}
\begin{tabbing}
E\=x\=amples:\\
...
Copyright \copyright\ 2009 Willow Garage
\vspace{3000 mm}
