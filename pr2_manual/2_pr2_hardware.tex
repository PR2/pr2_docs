...
The entire PR2 robot can be roughly divided into the following main components:  head, torso lift, arms, base and casters.
The arms can be further subdivided into shoulder, upper arm, forearm and gripper.

A detailed kinematic and dynamic description of the PR2 including CAD approximated inertia matrices can be found in the \href{http://www.ros.org/wiki/pr2_description}{pr2\_description} package.
...
\hline \hline
$\ast$\_caster\_rotation\_joint        & -     & 0.65  \\
$\ast$\_caster\_wheel\_$\ast$\_joint   & -     & 0.65  \\
...
The position limits for the PR2 are specified below. These "hard limits" are the maximum travel for the mechanism. On the PR2, "soft limits" stop the joints from reaching the full range of motion to prevent damage to the mechanism. These soft limits, similar to a virual spring, are specified in the robot's URDF file. For an explanation of their implementation, see http://www.ros.org/wiki/pr2_controller_manager/safety_limits.
...
r_shoulder\_pan\_joint                 & revolute   & 40$^\circ$   & 130$^\circ$  \\
l_shoulder\_pan\_joint                 & revolute   & 130$^\circ$  & -40$^\circ$  \\
$\ast$\_shoulder\_lift\_joint          & revolute   & 80$^\circ$   & 30$^\circ$  \\
r_upper\_arm\_roll\_joint              & revolute   & 44$^\circ$   & -224$^\circ$  \\
l_upper\_arm\_roll\_joint              & revolute   & 224$^\circ$  & -44$^\circ$  \\
