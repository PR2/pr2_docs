...
\item \texttt{10.68.0.0}: Robot Internal Network 1. The
  ...
  to easily put their laptop on the robot internal network.  The
  master computer, if booted, will give out IP addresses via DHCP in
  the range \texttt{10.68.0.100-10.68.0.199}.  Important addresses on
  this network:
  \begin{itemize}
...
\item \texttt{10.68.X.0}: Robot VPN Network. The primarily role of the
  ...
  be configured to forward to automatically forward relevant traffic
  from the building network into the VPN network, hiding this from the
...
\item \textbf{Direct wired connection}.  One possibility is to connect
  ...
  port, you can talk to the robot on its lan ports directly.  This
  system will work even if the VPN system is not functioning.  This is
  typically used for reinstalling the robot, or debugging other networking
  problems.
...
building network.  The robot will attempt to acquire an IP address via
...
The ESSID of this network defaults to <ROBOTNAME>LAN, and allows
...
not be able to see the outside world.
...
overlay can be found in the directory \texttt{/slave} on the master
machine.  Files added here will show up in the appropriate location on
c2.  For more information on how this is set up, see the man page for
...
\item \texttt{c1:/dev/sda5} -- \texttt{c1:/home} -- stores user home
  directories (linked to by \texttt{/u} by way of \texttt{/pr/1})
...
\item \texttt{c2:/dev/sda1} -- \texttt{c2:/home} -- stores user home
  directories (linked to by \texttt{/u} by way of \texttt{/pr/2})
\end{itemize}

Finally, both computers are configured to conveniently make use of the
additional removable drives.  Any drive loaded into the removable bayn
should always show up as \texttt{/dev/removable}.  The mount mount:
...
Robot pairing is performed with the the \texttt{robot-setup} command
...
Likewise, to disable forwarding you can run:
