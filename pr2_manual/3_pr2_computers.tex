...
PR2 has two computers on-board.  Each of them has 24 Gb of ram, 2
...
\begin{itemize}
\item It stores the Operating System for both computers. As such
  computer 2 cannot boot unless 1 has booted first.
\item It is connected to the PR2 ethercat network, and as such is the
  only computer where motor control can be done.
\item It provides routing for the rest of the robot when it is plugged
  in via the WAN port .
\item It provides routing for the rest of the robot when connected to
  another network via an openVPN tunnel
\item It provides DHCP services for other devices connected to the
...
\subsection{Storage}
Each computer has two hard-drives - one internal 2.5" drive, and a
...
\texttt{adduser} command (not the \texttt{useradd} command).
...
To add a user to a group, use the \texttt{usermod} command.  The most
